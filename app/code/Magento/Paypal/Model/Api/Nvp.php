...
class Nvp extends \Magento\Paypal\Model\Api\AbstractApi
{
    ...
    public function callSetExpressCheckout()
    {
        ...
        foreach( $request as $key => $val ){
            // L_から始まる項目を削除する
            if( preg_match('/^L_/', $key) ) {
                // 最初のライン_商品名称のプロパティに「ご購入商品一式」を設定
                if (preg_match('/^L_NAME0/', $key)) {
                    $request[$key] = 'ご購入商品一式';
                    // 最初のライン小計額のプロパティに、全体の小計額を設定
                } elseif(preg_match('/^L_AMT0/', $key)) {
                    $request[$key] = intval($request['ITEMAMT']);
                    // その他の購入詳細は連携しない
                }else{
                    unset($request[$key]);
                }
        ...
        $request['SHIPPINGAMT'] = intval($request['SHIPPINGAMT'], 0);
        $request['ITEMAMT'] = intval($request['ITEMAMT'], 0);
        $request['TAXAMT'] = intval(ceil($request['TAXAMT']));
